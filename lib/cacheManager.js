			...
			csv.parse(res[0].body, {}, function(err, output){
				...
				_.forEach(_.rest(output), function(game, index) {
					var startTime = moment.tz(game[0] + ' ' + game[2], "MM/DD/YY hh:mm A", "America/New_York"),
						...
						homeTeamId = teamIdsByName[gameNameArray[0]],
						awayTeamId = teamIdsByName[gameNameArray[1]],
					...
					//separate scheduled games into past and future arrays
				...
				//We only want the last 5 past games
			...
			_.forEach(config.activeTeams, function(teamId){
				_.forEach(cache.teams[teamId].schedule, function(game){
					...
					if(startTime.isBefore(now)){
						if(!game.linescore || (game.linescore.status !== "Final" && game.linescore.status !== "Completed Early")){
							...
							getBoxscore(game).then(function(boxscore){
