						...
						if(!game.linescore){
							...
							Q.all([
								...
								/*cache.teams[teamId].currentGame = _.assign(game, {
									linescore: require('../live_linescore.json')
								});*/
							});
						}
						else if(game.linescore.status === 'Manager Challenge' || game.linescore.status === 'Warmup' || game.linescore.status === 'In Progress' || game.linescore.status === 'Game Over'){
							console.log('Refreshing cache for game ' + game.name + ' at ' + game.startTime.format() + ' with status: ' + ((game.linescore) ? game.linescore.status : 'N/A'));
